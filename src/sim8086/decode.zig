...
pub fn decode_next_instruction(bytecode: []const u8) !Instruction {
    ...
    var current_byte = bytecode[0];

    // TODO: Test if inlining this for loop makes it faster or if the compiler inlines it for us
    for (instruction_map, 0..) |inst, i| {
        const op_found = current_byte >> @intCast(inst[0].position) == inst[0].payload;
    ...
    for (inst_fields[1..]) |field| {
        ...
        if (field.num_bits > 0) {
            if (bit_pos == 0) {
                read_pos += 1;
                current_byte = bytecode[read_pos];
                bit_pos = 8;
            }
        ...
        if (field.inst_type == .LITERAL) {
            assert(
                data == field.payload,
                "Required literal not found at pos byte: {d}, bit: {d}: 0b{b:0>8} got 0b{b:0>8}",
                .{ read_pos, bit_pos, field.payload, data },
            );
        } else {
            info[location] = data;
        }
    ...
    read_pos += @intCast(@intFromBool(bit_pos == 0));
    ...
    if (set[reg_as_int]) {
        instruction.operands[d ^ 1] = Operand{ .register = reg };
    }

    if (set[mod_as_int]) {
        if (mod == 0b11) {
            instruction.operands[d] = Operand{ .register = rm };
        } else {
